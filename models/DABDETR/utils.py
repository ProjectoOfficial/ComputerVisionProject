...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    wo_class_error=False, lr_scheduler=None, amp: bool = True, logger=None, scalar: int = 5, ema_m=None,
                    use_dn : bool = True, label_noise_scale: float = 0.2, box_noise_scale: float = 0.4,
                    num_patterns: int = 0, debug: bool = False):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        with torch.cuda.amp.autocast(enabled=amp):
            if need_tgt_for_training:
                outputs, mask_dict = model(samples, dn_args=(targets, scalar, label_noise_scale,
                                                             box_noise_scale, num_patterns))
