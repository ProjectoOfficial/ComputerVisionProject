...
class FasterRCNNBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, rpn, roi_heads, transform) -> None:
    ...
    def forward(self, images, targets = None):
        ...
        if self._train is not None and targets is None:
            raise ValueError("you must pass targets in training mode")

        original_image_sizes = torch.jit.annotate(List[Tuple[int, int]], [])
        
        for img in images:
            (height, width) = img.shape[-2:]
            original_image_sizes.append((height, width))
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        assert isinstance(rpn_anchor_generator, (AnchorBoxesGenerator, type(None)))
